# Wave-propagation data + full-system Houbolt / DifferentialEquations
using Test
using DrWatson
@quickactivate :BEM
using LinearAlgebra
...
@testset "Houbolt does not corrupt H" begin
    ...
    @test all(isfinite, dad.T[:, 1:3])  # startup may be zero
end

@testset "full-system build_wave_ode" begin
    dad, _ = _assemble_bar(; ndiv=8, n_int=4)
    prob, p = build_wave_ode(dad; tspan=(0.0, 0.2))
    @test hasproperty(p, :A) && hasproperty(p, :M)
    @test size(p.A, 1) == dad.nt
    ddu = zeros(dad.nt)
    wave_full_rhs!(ddu, zeros(dad.nt), zeros(dad.nt), p, 0.0)
    @test all(isfinite, ddu)
    # Dirichlet ü forced to 0
    for i in 1:dad.n
        dad.BC[i] == 0 && @test ddu[i] == 0
    end
end
