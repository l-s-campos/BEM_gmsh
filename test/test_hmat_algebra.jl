# H-matrix algebra: matvec, hmul, hlru, hadd, hara, h2 orthog
using Test
using LinearAlgebra
...
@testset "hara from kernel sampler" begin
    pts = _laplace_pts(8)
    K = _laplace_kernel(pts)
    tree = ClusterTree(pts, PrincipalComponentSplitter(; nmax=12))
...
@testset "h2_orthog! preserves matvec" begin
    pts = _laplace_pts(10)
    K = _laplace_kernel(pts)
    tree = ClusterTree(pts, PrincipalComponentSplitter(; nmax=16))
    ...
    y0 = H2 * x
    h2_orthog!(H2)
    h2_compress!(H2; rtol=1e-5)
    y1 = H2 * x
    rel = norm(y1 - y0) / (norm(y0) + 1e-14)
    @test rel < 0.05
end
