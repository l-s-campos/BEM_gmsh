#!/usr/bin/env julia
# Pinheiro thesis §8.2.1 — MECID alternative, variable velocity (div v = 0)
...
function main()
    println("="^72)
    println("MECID Alternative — C8E1 (Pinheiro Ch.8 §8.2.1)")
    ...
    for n_int in (3, 5, 7, 9)
        ...
        dad = setup_mecid_c8e1(msh; m=1.0, n_int=n_int)
        res = test_mecid_c8e1(dad; m=1.0, npg=14, verbose=false)
    ...
    for m in 1:6
        ...
        dad = setup_mecid_c8e1(msh; m=float(m), n_int=7)
        res = test_mecid_c8e1(dad; m=float(m), npg=14, verbose=false)
