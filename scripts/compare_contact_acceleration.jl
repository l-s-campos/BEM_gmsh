...
# Plot matvec time
fig1 = Figure(size=(480, 360), backgroundcolor=:white)
ax1 = Axis(fig1[1, 1];
    xlabel="N", ylabel="matvec time [s]",
    xscale=log2, yscale=log10,
    title="Half-space matvec cost",
...
for m in (:dense, :fft, :hmatrix, :fmm)
    ...
    lines!(ax1, r.n, r.t_mv; color=cols[m], label=String(m))
    scatter!(ax1, r.n, r.t_mv; color=cols[m], marker=marks[m])
end
# reference slopes
nref = [64.0, 1024.0]
lines!(ax1, nref, 1e-7 .* nref .^ 2; color=:gray, linestyle=:dash, label="O(N²)")
lines!(ax1, nref, 3e-8 .* nref .* log2.(nref); color=:gray, linestyle=:dot, label="O(N log N)")
axislegend(ax1; position=:lt, framevisible=true, labelsize=9)
save(joinpath(OUTDIR, "fig_matvec_scaling.pdf"), fig1)
save(joinpath(OUTDIR, "fig_matvec_scaling.png"), fig1)
println("  saved fig_matvec_scaling.pdf")

# Plot relative error
fig2 = Figure(size=(480, 360), backgroundcolor=:white)
ax2 = Axis(fig2[1, 1];
    xlabel="N", ylabel="relative matvec error",
    xscale=log2, yscale=log10,
    title="Accuracy vs dense reference",
)
for m in (:fft, :hmatrix, :fmm)
    r = results[m]
    isempty(r.n) && continue
    lines!(ax2, r.n, max.(r.err, 1e-16); color=cols[m], label=String(m))
    scatter!(ax2, r.n, max.(r.err, 1e-16); color=cols[m], marker=marks[m])
end
axislegend(ax2; position=:rt, framevisible=true, labelsize=9)
save(joinpath(OUTDIR, "fig_matvec_error.pdf"), fig2)
save(joinpath(OUTDIR, "fig_matvec_error.png"), fig2)
println("  saved fig_matvec_error.pdf")

# Memory
fig3 = Figure(size=(480, 360), backgroundcolor=:white)
ax3 = Axis(fig3[1, 1];
    xlabel="N", ylabel="memory [MB]",
    xscale=log2, yscale=log10,
    title="Operator storage",
)
for m in (:dense, :fft, :hmatrix, :fmm)
    r = results[m]
    isempty(r.n) && continue
    lines!(ax3, r.n, max.(r.mem, 1e-4); color=cols[m], label=String(m))
    scatter!(ax3, r.n, max.(r.mem, 1e-4); color=cols[m], marker=marks[m])
end
axislegend(ax3; position=:lt, framevisible=true, labelsize=9)
save(joinpath(OUTDIR, "fig_memory.pdf"), fig3)
save(joinpath(OUTDIR, "fig_memory.png"), fig3)
...
fig4 = Figure(size=(480, 360), backgroundcolor=:white)
ax4 = Axis(fig4[1, 1]; xlabel="x", ylabel="p(x)", title="Line contact pressure")
lines!(ax4, dad.x, p; color=:dodgerblue, label="BEM (FFT)")
lines!(ax4, dad.x, p_hz; color=:black, linestyle=:dash, label="Hertz")
axislegend(ax4; position=:rt, framevisible=true, labelsize=9)
save(joinpath(OUTDIR, "fig_hertz_pressure.pdf"), fig4)
save(joinpath(OUTDIR, "fig_hertz_pressure.png"), fig4)
...
fig5 = Figure(size=(480, 360), backgroundcolor=:white)
ax5 = Axis(fig5[1, 1]; xlabel="x", ylabel="wear depth", title="Archard wear evolution")
steps_show = [1, 10, 20, 40]
cmap = Makie.cgrad(:viridis, length(steps_show))
for (k, s) in enumerate(steps_show)
    s <= size(w1, 2) || continue
    lines!(ax5, dad.x, w1[:, s] .+ w2[:, s]; color=cmap[k], label="step $s")
end
axislegend(ax5; position=:rt, framevisible=true, labelsize=9)
save(joinpath(OUTDIR, "fig_wear_evolution.pdf"), fig5)
save(joinpath(OUTDIR, "fig_wear_evolution.png"), fig5)
println("  saved fig_wear_evolution.pdf")

fig6 = Figure(size=(480, 360), backgroundcolor=:white)
ax6 = Axis(fig6[1, 1]; xlabel="x", ylabel="p(x)", title="Pressure during wear")
for (k, s) in enumerate(steps_show)
    s <= size(ph, 2) || continue
    lines!(ax6, dad.x, ph[:, s]; color=cmap[k], label="step $s")
end
axislegend(ax6; position=:rt, framevisible=true, labelsize=9)
save(joinpath(OUTDIR, "fig_wear_pressure.pdf"), fig6)
save(joinpath(OUTDIR, "fig_wear_pressure.png"), fig6)
...
fig7 = Figure(size=(480, 360), backgroundcolor=:white)
ax7 = Axis(fig7[1, 1];
    xlabel="N", ylabel="contact solve time [s]",
    xscale=log2, yscale=log10,
    title="Force-controlled contact solve",
)
for m in (:dense, :fft, :hmatrix, :fmm)
    ts = t_contact[m]
    ok = findall(isfinite, ts)
    isempty(ok) && continue
    lines!(ax7, ns2[ok], ts[ok]; color=cols[m], label=String(m))
    scatter!(ax7, ns2[ok], ts[ok]; color=cols[m], marker=marks[m])
end
axislegend(ax7; position=:lt, framevisible=true, labelsize=9)
save(joinpath(OUTDIR, "fig_contact_solve_time.pdf"), fig7)
save(joinpath(OUTDIR, "fig_contact_solve_time.png"), fig7)
