...
function apply_farfield!(mesh, σ)
    for el in mesh.elements
        if el.eq_type == 1
            for loc in 1:3
                n = mesh.nodes[el.fis[loc]].normal
                if abs(n[2]) > 0.7
                    el.bc_type[loc, :] .= 1
                    el.bc_val[loc, 1] = 0.0
                    el.bc_val[loc, 2] = sign(n[2]) * σ
                end
            end
        elseif el.eq_type in (2, 3)
            # crack faces: free of prescribed traction (cohesive provides it)
            el.bc_type .= 1
            el.bc_val .= 0.0
        end
    end
...
function main()
    ...
    println("nodes=$(length(mesh.nodes))  elems=$(length(mesh.elements))")
