# 12×6 Laplace bar, Houbolt Δt=1, 4 periods. CBIE only.
# DRM/DIBEM: PHS3 φ=r³ + linear (1,x,y). Cells: volume quadrature.
#
#   julia --project=. scripts/plot_burton_miller_bar12x6.jl
using DrWatson
@quickactivate :BEM
...
const Tper = 4 * Lx          # fundamental period 4L/c, c=1
const Δt = 1.0
...
function mesh_bar_12x6(; ndivx=13, ndivy=7, ordem=1, nome="bm_bar12x6")
    gmsh.initialize()
    ...
    gmsh.model.addPhysicalGroup(1, [l2], -1, "1;-1")      # sudden unit load
    gmsh.model.addPhysicalGroup(1, [l4], -1, "0;0")       # fixed
...
# ---------------------------------------------------------------------------
println(" Laplace  12×6 bar  Houbolt  Δt=$Δt  4 periods (tf=$tf)")
println(" RBF: PHS3  φ=r³  poly_deg=1  (1, x, y)  — no Burton–Miller")
dad0 = make_dad()
@printf(" elements=%d  n=%d  ni=%d  nt=%d  cells=%d\n",
    length(dad0.elements), dad0.n, dad0.ni, dad0.nt,
...
labs = Dict(:drm => "DRM", :dibem => "DIBEM", :cells => "cells")
runs = Dict{Symbol,NamedTuple}()

@printf("\n  %-8s  %10s  %10s  %10s  %s\n",
    "mass", "u_end_err", "max|u|", "blow", "finite")
for mass in MASSES
    dad = deepcopy(dad0)
    mass!(dad, mass)
    solve_Houbolt(dad, Δt, tf)
    ux, _ = probe_u(dad.T, dad)
    t = dad.time
...
plot!(plt; xlabel=L"t", ylabel=L"u(L, H/2)", ylim=(-2, 28),
    legend=:outertopright,
    title="12×6 bar  Laplace  PHS3+linear  Houbolt  \$\\Delta t=1\$  4 periods")

mkpath(joinpath(projectdir(), "plots"))
out = joinpath(projectdir(), "plots", "bar12x6_laplace_phs3_dt1")
savefig(plt, out * ".png")
savefig(plt, out * ".pdf")
