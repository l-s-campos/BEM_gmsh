...
@testset "Cohesive / mortar history residual at C" begin
    par = loyola_cattaneo_params()
    ν = par.ν
    G = par.E_eq * (1 - ν) / 2
    x = collect(range(-3par.a, 3par.a; length=81))
    hp = ElasticHalfPlane2D(G, ν; h=x[2] - x[1])
    # cohesive history: unload C must keep residual shear
    coh = solve_cattaneo_cohesive_history_halfplane(
        x, par.R_eq, par.f, hp, par.load_steps; η_coh=0.05, tol=1e-8)
    solC = coh[findfirst(s -> s.name === :C, coh)]
    @test solC.Q ≈ 0 atol = 1e-14
    @test maximum(abs, solC.τ) > 0.05 * par.p0
    # mortar history on non-matching grids
    xm = collect(range(-3par.a, 3par.a; length=31))
    mort = solve_cattaneo_mortar_history_halfplane(
        x, xm, par.R_eq, par.f, G, ν, par.load_steps; tol=1e-8)
    solCm = mort[findfirst(s -> s.name === :C, mort)]
    @test maximum(abs, solCm.τ_s) > 0.05 * par.p0
    @test maximum(abs, solCm.τ_m) > 0.02 * par.p0
end
