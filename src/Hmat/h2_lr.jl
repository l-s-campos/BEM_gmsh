...
function _h2_clone_node(N::H2Node{R, T}, pack::H2Pack{R, T}) where {R, T}
    ...
    elseif isuniform(N)
        ...
        return _h2node_uniform(N.row_id, N.col_id, S, pack)
    else
        rs, cs = size(N.sons)
...
function _h2_uniform_dense(N::H2Node{R, T}) where {R, T}
    ...
    Sm = N.S isa RkMatrix ? Matrix(N.S) : Matrix{T}(N.S)
...
"""
    h2_addmul!(C, A, B, α=1; rtol=0)

`C ← C + α*A*B` on recursive H² nodes (H2Lib `addmul_h2matrix` MVP).

When all three nodes share a compatible split, updates son-wise; otherwise
densifies, GEMMs, and writes a dense leaf into `C`.
"""
function h2_addmul!(
        C::H2Node{R, T},
        A::H2Node{R, T},
    ...
    if issplit(C) && issplit(A) && issplit(B) &&
        ...
        for i in 1:rC, k in 1:cC
            for j in 1:mid
                h2_addmul!(C.sons[i, k], A.sons[i, j], B.sons[j, k], α; rtol = rtol)
            end
        end
        return C
    end
    ...
    Bd = h2_block_matrix(B)
    mul!(Cd, Ad, Bd, α, true)
    if rtol > 0 && min(size(Cd)...) > 4
        # optional low-rank rewrite for far-looking blocks (keep dense for stability)
        # leave dense; rtol reserved for future uniform recompress
    end
