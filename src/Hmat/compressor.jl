...
function _aca_partial_initial_pivot(rowtree)
    ...
    # For ExpandedClusterTree (scalarized tensors), search on the underlying
    # point tree and map back to the first scalar index of that point.
    if rowtree isa ExpandedClusterTree
        ipt = _aca_partial_initial_pivot(rowtree.tree)
        return scalar_index(ipt, 1, rowtree.p)
    end
