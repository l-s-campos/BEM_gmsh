...
"""`y = P*x` with `P[i,j] = data[rowperm[i], colperm[j]]` via one matvec on `data`."""
function LinearAlgebra.mul!(y::AbstractVector, P::PermutedMatrix, x::AbstractVector)
    length(x) == size(P, 2) && length(y) == size(P, 1) || throw(DimensionMismatch())
    xp = similar(x)
...
Base.:*(P::PermutedMatrix, x::AbstractVector) = mul!(similar(x, eltype(P)), P, x)

function LinearAlgebra.mul!(y::AbstractVector, Pt::Adjoint{<:Any,<:PermutedMatrix}, x::AbstractVector)
    P = parent(Pt)
    length(x) == size(P, 1) && length(y) == size(P, 2) || throw(DimensionMismatch())
    # (P*x)_i uses data[rp[i], cp[j]]; P' uses data[rp[j], cp[i]]'
    xp = similar(x)
