...
"""
    _aca_column_id(A; rtol=1e-6, rank=typemax(Int), atol=0)

Partial ACA on dense `A`, returning interpolative `(sk, rd, T)` so
`A[:, rd] ≈ A[:, sk] * T` (same contract as [`interpolative_decomp`](@ref)).
Column pivots of the crosses are the skeleton.
"""
function _aca_column_id(A::AbstractMatrix{T}; rtol = 1e-6, rank = typemax(Int),
        atol = 0.0) where {T}
    m, n = size(A)
    ...
    Jok = trues(n)
    sk = Int[]
    sizehint!(sk, min(rmax, n))
    Ucols = Vector{Vector{T}}()
    Vcols = Vector{Vector{T}}()
    i = 1
    r = 0
    er = Inf
    est = 0.0
    τabs = float(atol)
    τrel = float(rtol)
    while r < rmax && er > max(τabs, τrel * est)
        found = false
        while i !== nothing && 1 <= i <= m
            Iok[i] || break
            Iok[i] = false
            b = Vector{T}(undef, n)
            ...
            for k in 1:r
                axpy!(-adjoint(Ucols[k][i]), Vcols[k], b)
            end
            j = 0
            best = zero(T)
            @inbounds for jj in 1:n
            ...
            for k in 1:r
                axpy!(-adjoint(Vcols[k][j]), Ucols[k], a)
            end
            r += 1
            push!(Ucols, a)
            push!(Vcols, b)
    ...
    rd = findall(Jok)
    k = length(sk)
    k == 0 && return Int[], collect(1:n), zeros(T, 0, n)
    Tm = isempty(rd) ? zeros(T, k, 0) : Matrix{T}(A[:, sk]) \ Matrix{T}(A[:, rd])
    return sk, rd, Tm
end
...
function _aca_partial_initial_pivot(rowtree)
    # Bebendorf: initial pivot ≈ point closest to the cluster center.
