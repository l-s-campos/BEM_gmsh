"""
    mutable struct HMatrix{R,T} <: AbstractStructuredMatrix{T}
...
# Admissibility types live in admissibility.jl
...
"""
    plot_hmatrix(H; figsize=(700, 650), title="H-matrix block structure") -> Plots.Plot

Block-structure plot of an [`HMatrix`](@ref) using Plots (GR backend).
"""
function plot_hmatrix(
        hmat::HTypes;
        figsize=(700, 650),
        title="H-matrix block structure",
        show_rank=false,
    )
    nrows = length(rowtree(hmat))
    ncols = length(coltree(hmat))
    ...
    # legend proxies
    scatter!(plt, [NaN], [NaN]; color=:crimson, marker=:square, markersize=8, label="dense", alpha=0.5)
    scatter!(plt, [NaN], [NaN]; color=:dodgerblue, marker=:square, markersize=8, label="low-rank", alpha=0.7)

    for block in leaves(hmat)
        mat = data(block)
        pt1 = pivot(block)
        sz = size(block)
        x1 = Float64(pt1[2]) - 0.5
        y1 = Float64(pt1[1]) - 0.5
        x2 = Float64(pt1[2] + sz[2]) - 0.5
        y2 = Float64(pt1[1] + sz[1]) - 0.5
        xs = [x1, x2, x2, x1, x1]
        ys = [y1, y1, y2, y2, y1]

        if mat isa Adjoint
            plot!(plt, xs, ys; seriestype=:shape, color=:gray, alpha=0.12,
                linecolor=:black, linewidth=0.4, label="")
        elseif isadmissible(block)
            ...
            m, n = size(mat)
            ratio = m * n / (r * (m + n) + eps())
            α = clamp(1 / ratio, 0.08, 0.95)
            plot!(plt, xs, ys; seriestype=:shape, color=:dodgerblue, alpha=α,
                linecolor=:black, linewidth=0.4, label="")
            if show_rank && min(m, n) > 8
                annotate!(plt, (x1 + x2) / 2, (y1 + y2) / 2, text(string(r), 8, :white))
            end
        else
            plot!(plt, xs, ys; seriestype=:shape, color=:crimson, alpha=0.35,
                linecolor=:black, linewidth=0.4, label="")
        end
    end
    return plt
