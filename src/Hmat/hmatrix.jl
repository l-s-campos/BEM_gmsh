...
"""
    plot_hmatrix(H; figsize=(700, 650), title="H-matrix block structure") -> Figure

Block-structure plot of an [`HMatrix`](@ref) using **Makie** (same stack as BEM).
"""
function plot_hmatrix(
    ...
    for block in leaves(hmat)
        ...
        if mat isa Adjoint
            poly!(ax, rect; color=(:gray70, 0.12), strokecolor=:black, strokewidth=0.4)
        elseif isadmissible(block)
            ...
            poly!(ax, rect; color=(:dodgerblue, α), strokecolor=:black, strokewidth=0.4)
            if show_rank && min(m, n) > 8
                text!(
                    ax, (x1 + x2) / 2, (y1 + y2) / 2;
                    text=string(r), align=(:center, :center), fontsize=9, color=:white,
                )
            end
        else
            poly!(ax, rect; color=(:crimson, 0.35), strokecolor=:black, strokewidth=0.4)
        end
    end

    xlims!(ax, 0.5, ncols + 0.5)
    ylims!(ax, 0.5, nrows + 0.5)
    scatter!(ax, [NaN], [NaN]; color=(:crimson, 0.35), marker=:rect, markersize=14, label="dense")
    scatter!(ax, [NaN], [NaN]; color=(:dodgerblue, 0.7), marker=:rect, markersize=14, label="low-rank")
