# Nested H² low-rank update (H2Lib rkupdate / Börm–Reimer R2–R3 MVP)
#
# G ← G + X Y'
#   1. Snapshot full leaf blocks of G
#   2. Expand nested pack.U so leaves contain range([X Y])
#   3. Rebuild non-leaf transfers (bottom-up ONB of children)
#   4. Rewrite each far leaf as nested S = Vi'(Mold + Xi Yj')Vj, or s_full fallback
#   5. Dense leaves: F += Xi Yj'
#   6. Optional weighted recompress of bases + project S
#   7. Write pack.Bfar / D* from nodes

# ---- weights -----------------------------------------------------------------

"""
    struct H2ClusterOperator{T}

Weight tree for nested recompression (H2Lib `clusteroperator` MVP).
"""
struct H2ClusterOperator{T}
    tidx::H2TreeIndex
    C::Vector{Matrix{T}}
...
Frobenius-normalized weights from nested far couplings.
"""
function prepare_h2_weights(pack::H2Pack{R, T}; side::Symbol = :row) where {R, T}
    ...
    return H2ClusterOperator{T}(tidx, C)
end

_h2_as_matrix(S::RkMatrix) = Matrix(S)
_h2_as_matrix(S::AbstractMatrix) = copy(S)
_h2_as_matrix(_) = zeros(Float64, 0, 0)

# ---- nested rkupdate ---------------------------------------------------------

"""
    h2_rkupdate_nested!(G, X, Y; rtol=1e-6, recompress=true, rank=typemax(Int))

Nested-basis ``G ← G + X Y'`` with basis expansion + optional weighted recompress.
"""
function h2_rkupdate_nested!(
    ...
    # --- snapshot full leaf blocks (ambient coordinates) ---
    ...
    h2_foreach(G) do node
        ...
        # Far: try nested coupling in new bases
        ...
        if size(Vi, 2) > 0 && size(Vj, 2) > 0 &&
            ...
            rel = norm(recon - Mold) / max(norm(Mold), eps(T))
        ...
        # Fallback: full-block low-rank / dense
        m, nblk = size(Mold)
    ...
    # --- optional weighted recompress of nested (non s_full) structure ---
    if recompress && float(rtol) > 0
        try
            ...
            h2_foreach(G) do node
                ...
                S = pack.Bfar[(i, j)]
                pack.Bfar[(i, j)] = _h2_proj_S(S, Rproj[i], Rproj[j])
                node.S = pack.Bfar[(i, j)]
            end
            # rebuild transfers after leaf trunc already done inside weighted_recompress
            _h2_rebuild_transfers!(pack)
            # re-sync nested S ranks if bases changed
            h2_foreach(G) do node
                isuniform(node) && !node.s_full || return
                i, j = node.row_id, node.col_id
                haskey(pack.Bfar, (i, j)) || return
            ...
            # keep expanded form without extra trunc
        end
    end
...
function _h2_fit_transfer(Vch::Vector{<:AbstractMatrix{T}}, Vp::AbstractMatrix{T}) where {T}
    ...
    for (Vc, r, m) in zip(Vch, rs, ms)
        if r > 0 && m > 0 && size(Vp, 2) > 0
            # thin ON child basis: E_block = Vc' * Vp_block
