...
function _allocate_ki(root::ClusterTree{3}, n::Int, ::Type{T}) where {T}
    ...
    nn = nnodes(root)
    data = Vector{KINodeData{T}}(undef, nn)
    for node in nodes(root)
        ctr = SVector{3,Float64}(center(container(node)))
        r = max(diameter(node) / 2, 1e-30)
        data[node_id(node)] = KINodeData{T}(zeros(T, n), zeros(T, n), ctr, r)
    end
    return data
end

_kiget(data::AbstractVector, node) = data[node_id(node)]
_kiget(data::AbstractDict, node) = data[objectid(node)]
...
function kifmm3d(
    ...
    function run(tmat, is_src)
        ...
        function m2l_job!(tnode, snode, _, __)
            td = _kiget(tdata, tnode)
            sd = _kiget(sdata, snode)
