...
"""Charge-only P2P on SoA coordinates (Flatiron `r2d_directcp`: `log(r²)/2`).
`exclude_self` only for the same leaf; other pairs skip the `i==j` test."""
function direct_laplace_soa!(
    pot::AbstractVector{<:Real},
    xs::Vector{Float64},
    ys::Vector{Float64},
    charges::AbstractVector{Float64},
    ...
    if exclude_self
        @inbounds for j in targ_range
            ...
            for i in src_range
                ...
                dx = tx - xs[i]
                dy = ty - ys[i]
                r2 = muladd(dy, dy, dx * dx)
                r2 <= 1e-30 && continue
                pj = muladd(charges[i], log(r2) * 0.5, pj)
            end
            pot[j] += pj
        end
    else
        @inbounds for j in targ_range
            ...
            ty = ys[j]
            pj = 0.0
            @simd for i in src_range
                dx = tx - xs[i]
                dy = ty - ys[i]
                r2 = muladd(dy, dy, dx * dx)
                pj = muladd(charges[i], log(max(r2, 1e-300)) * 0.5, pj)
            end
            pot[j] += pj
        end
    end
