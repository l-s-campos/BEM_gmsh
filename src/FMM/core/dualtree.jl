...
_edata(data::AbstractVector, node) = data[node_id(node)]
_edata(data::AbstractDict, node) = data[objectid(node)]

"""Map each leaf `node_id` → `view(buf, index_range(leaf))` (nothing elsewhere)."""
function leaf_views(tree::ClusterTree, buf::AbstractVector)
    if node_id(tree) == 0
        assign_node_ids!(tree)
    end
    m = Vector{Any}(undef, nnodes(tree))
    fill!(m, nothing)
    for leaf in leaves(tree)
        m[node_id(leaf)] = view(buf, index_range(leaf))
    end
    return m
end

"""Dense multipole map for leaf-only algorithms: `node_id → vector`."""
function leaf_mp_map(tree::ClusterTree, make_mp)
    if node_id(tree) == 0
        assign_node_ids!(tree)
    end
    m = Vector{Any}(undef, nnodes(tree))
...
function build_point_tree(pts::AbstractMatrix{<:Real}, spl)
    ...
    tree = ClusterTree(sv, spl; copy_elements=false, cube=cube)
